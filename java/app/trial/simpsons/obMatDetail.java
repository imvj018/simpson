...
public class obMatDetail extends AppCompatActivity implements View.OnClickListener {

    TextView matnum, matdesc, bin, qty;
    Button qrcode;
    String Id, Rid, Itemno, Material, Batch, Bin, Quantity, Wno, Sled, Status, Uom, Description;
    String obcid, obchange, obcreate, obdorder, obrefer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        obcid = intent.getStringExtra("CID");
        obchange = intent.getStringExtra("CHANGEDATE");
        obdorder = intent.getStringExtra("DORDERNO");
        obcreate = intent.getStringExtra("CREATEDATE");
        obrefer = intent.getStringExtra("REFER");
        ...
        if (result != null) {
            if (result.getContents() != null) {
                if (result.getContents().equals(Bincheck)) {
                    ...
                    builder.setPositiveButton("Scan Group", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            Intent intent = new Intent(obMatDetail.this, obMatscanDetail.class);
                            ...
                            intent.putExtra("OBCID", obcid);
                            intent.putExtra("OBCREATE", obcreate);
                            intent.putExtra("OBCHANGE", obchange);
                            intent.putExtra("OBDORDER", obdorder);
                            intent.putExtra("OBREFER", obrefer);

                            startActivity(intent);
                        }
                    }).setNegativeButton("Scan Individual", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            Intent intent = new Intent(obMatDetail.this, obMatIdetail.class);
                            intent.putExtra("ID", Id);
                            intent.putExtra("WNO", Wno);
                            intent.putExtra("RID", Rid);
                            intent.putExtra("ITEMNO", Itemno);
